    ...
    visitor: {
      'ClassDeclaration|ClassExpression' (path, state) {
        if (path.node.superClass.object.name === 'React') {
          ...
          //   const id = path.scope.generateUidIdentifierBasedOnNode(path.node.id || 'class')
          //   path.remove()
          //   path.scope.parent.push({ id, init: path.node })
          // }
          // console.log(path.parentPath.scope)

          const program = path.parentPath.scope.path.get('body')
          console.log(program)
          program.pushContainer(
            ...
            t.assignmentExpression(
              ...
              t.memberExpression(
                ...
                t.identifier('_detectedProps')
              ),
              t.stringLiteral('A little high, little low.')
            )
          )

          // console.log(t.isClassBody(body))
          // body.pushContainer(
          //   'body',
          //   t.classProperty(
          //     t.identifier('test'),
